...
export function Footer({handlePhotoAdd}: Props) {
    ...
    return (<footer className="flex-none w-full h-24 flex items-center justify-center p-4 border-t-2 border-slate-300">
        {!path && <UploadPhotoButton onChange={setPath}/>}
        {!!path && <ImageDataInput photoPath={path} handlePhotoAdd={(p) => {
            handlePhotoAdd(p);
            setPath(null);
        }}/>}
    </footer>);
